...
CONTAINS
 ...
 if (nUps>0) then
   ...
   do iUps = 1,nUps
     ...
     q_upstream = q_upstream + RCHFLX_out(iRch_ups)%ROUTE(idxDW)%REACH_Q
   end do
 endif
 ...
 call comp_reach_wb(NETOPO_in(segIndex)%REACHID, idxDW, q_upstream, RCHFLX_out(segIndex), verbose, lakeFlag=.false.)
