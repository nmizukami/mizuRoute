 ...
 CONTAINS
  ...
  SUBROUTINE write_history_state(ierr, message1)
    ...
    if (meta_rflx(ixRFLX%KWTroutedRunoff)%varFile) then
      ...
      call write_pnetcdf(pioFileDesc, 'KWTroutedRunoff', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif

    if (meta_rflx(ixRFLX%IRFroutedRunoff)%varFile) then
      array_dp = hVars%discharge(index_write, idxIRF)
      call write_pnetcdf(pioFileDesc, 'IRFroutedRunoff', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif

    if (meta_rflx(ixRFLX%KWroutedRunoff)%varFile) then
      array_dp = hVars%discharge(index_write, idxKW)
      call write_pnetcdf(pioFileDesc, 'KWroutedRunoff', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif

    if (meta_rflx(ixRFLX%MCroutedRunoff)%varFile) then
      array_dp = hVars%discharge(index_write, idxMC)
      call write_pnetcdf(pioFileDesc, 'MCroutedRunoff', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif

    if (meta_rflx(ixRFLX%DWroutedRunoff)%varFile) then
      array_dp = hVars%discharge(index_write, idxDW)
      call write_pnetcdf(pioFileDesc, 'DWroutedRunoff', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif

    if (meta_rflx(ixRFLX%IRFvolume)%varFile) then
      array_dp = hVars%volume(index_write, idxIRF)
      call write_pnetcdf(pioFileDesc, 'IRFvolume', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif

    if (meta_rflx(ixRFLX%KWvolume)%varFile) then
      array_dp = hVars%volume(index_write, idxKW)
      call write_pnetcdf(pioFileDesc, 'KWvolume', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif

    if (meta_rflx(ixRFLX%MCvolume)%varFile) then
      array_dp = hVars%volume(index_write, idxMC)
      call write_pnetcdf(pioFileDesc, 'MCvolume', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif

    if (meta_rflx(ixRFLX%DWvolume)%varFile) then
      array_dp = hVars%volume(index_write, idxDW)
      call write_pnetcdf(pioFileDesc, 'DWvolume', array_dp, ioDesc_rch_double, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif
