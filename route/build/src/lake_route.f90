...
CONTAINS
  ...
  ! *********************************************************************
  SUBROUTINE lake_route(segIndex,   &    ! input: index of runoff ensemble to be processed
...
!  real(dp)                                 :: WB             ! water balance component in the lake
  ...
  if(NETOPO_in(segIndex)%REACHIX == ixDesire)then
    ...
    !print*, 'volume before simulation m3.= ', RCHFLX_out(segIndex)%REACH_VOL(0)
    !print*, 'upstream streamflow m3/s ...= ', RCHFLX_out(segIndex)%REACH_Q
    !print*, 'upstream precipitation m3/s.= ', RCHFLX_out(segIndex)%basinprecip
    !print*, 'upstream evaporation m3/s ..= ', RCHFLX_out(segIndex)%basinevapo
...
!          if (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) < 0) then; ! set the lake volume as 0 if it goes negative
!            RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1)=0
!          endif
        case(hanasaki06)
          ...
          !print*, RCHFLX_out(segIndex)%QPASTUP_IRF
...
!    endif
    ...
    call comp_reach_wb(NETOPO_in(segIndex)%REACHID, idxRoute, q_upstream, RCHFLX_out(segIndex), &
